...
tools/absgenftml.py -q -t 'Subtending Marks (auto)' source/masters/Alkalami-Regular.ufo  tests/SubtendingMarks-auto.ftml -l tests/logs/SubtendingMarks-auto.log       -s 'local(Scheherazade New)|Sch (inst)' --ap "_?dia[AB]$" --xsl ../tools/ftml.xsl --scale 250 -i source/glyph_data.csv -s 'url(../references/Alkalami-Regular.ttf)|2.0' -s 'url(../results/Alkalami-Regular.ttf)|Reg'  &
wait
echo done.
